...
class NeuralNetEnhanced(nn.Module):
    def __init__(self, in_size, bs, head, lr, ftdim):
        super(NeuralNetEnhanced, self).__init__()
        torch.manual_seed(0)
        self.model = nn.Sequential(
                     nn.Linear(in_size, ftdim),
                     nn.ReLU(),  # Optional, for non-linearity
                     nn.Linear(ftdim, ftdim)
                    )
        self.optimizer = optim.SGD(self.model.parameters(), lr=lr, momentum=0.9)
        self.loss = torch.nn.BCELoss()
        self.bs = bs
    ...
    def attention_scores(self, x):
        ...
        attn_scores = torch.nn.functional.softmax(feature)
        return attn_scores
...
def learn_PIE(target_model,full_model,concept_mask,target_img,target_label,fc,lr,epochs,image_norm=None,net_type="orig"):
    ...
    elif net_type == "Enhanced":
        net = NeuralNetEnhanced(num_feat,bs,fc,lr,feat.shape[1]).cuda()
